...
class Verify {
    ...
    public function check($code, $id = '') {
        ...
        if($this->authcode(strtoupper($code)) == $secode['verify_code']) {
            $this->reset && Func::session($key, null);
            return true;
        }
    ...
    /**
     * 输出验证码并把验证码的值保存的Func::session中
     * 验证码保存到Func::session的格式为： array('verify_code' => '验证码值', 'verify_time' => '验证码创建时间');
     * @access public     
    ...
    public function entry($id = '') {
        ...
        $secode     =   array();
        $secode['verify_code'] = $code; // 把校验码保存到Func::session
        $secode['verify_time'] = $_SERVER['REQUEST_TIME'];  // 验证码创建时间
        Func::session($key.$id, $secode);
